...
from models import db, CustomField1

class CustomField1Resource(Resource):
    def get(self, id=None):
        if id:
            field = CustomField1.query.get(id)
            if not field:
                return {"message": "Field not found"}, 404
            return field.to_dict(), 200
        fields = CustomField1.query.all()
        return jsonify([field.to_dict() for field in fields])
    
    def post(self):
        data = request.get_json()
        new_field = CustomField1(
            name=data.get('name'),
    ...
    def put(self, id=None):
        if id:
            ...
            if not field:
                ...
                field=CustomField1.query.first()
                if not field:
                    return {"message": "No such field"}
